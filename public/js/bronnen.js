async function laadBronnen() {
  ...
  grid.innerHTML = bronnen.map(b => {
    ...
    <div class="bron-kaart ${isBezig ? 'bezig' : ''}" id="bronKaart_${b.id}">
      ...
      <div class="acties">
        ${knop}
        <button class="btn btn-secundair" onclick="bewerkBron(${b.id}, '${b.naam}', '${b.pad}', '${b.type}')" title="Bewerken">✏️</button>
...
async function voegBronToe() {
  ...
  await fetch('/api/bronnen', {
    ...
    body: JSON.stringify({ naam, type, pad, icoon: icoonen[type] })
  });
...
function bewerkBron(id, naam, pad, type) {
...
async function slaaBewerkingOp() {
  ...
  await fetch('/api/bronnen/' + id, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ naam, pad, type, icoon: icoonen[type] || '💻' })
  });
  sluitBewerkModal();
  laadBronnen();
}
